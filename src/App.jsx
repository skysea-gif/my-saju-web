...
function App() {
    ...
    // Gemini API 호출 함수
    const fetchFortuneWithAI = async (data) => {
        setView('loading');
        setErrorMsg('');
...
각 항목(detail)의 풀이는 **최소 10문장 이상, 500자 이상의 매우 긴 분량**으로 작성해야 하며, 원인 분석, 현재 상황의 특징, 그리고 앞으로 취해야 할 구체적인 행동 지침(솔루션)까지 기승전결이 완벽한 하나의 칼럼처럼 작성해주세요.`;
            ...
            const userPrompt = `이름: ${data.name}, 성별: ${data.gender}, 생년월일: ${data.calendar} ${data.birthDate}, 태어난 시간: ${data.birthTime || '모름'}. 이 사람의 사주팔자(년주, 월주, 일주, 시주)를 도출하고(태어난 시간을 모를 경우 시주는 '알수없음'으로 처리), 주어진 JSON 스키마에 맞춰 사주 풀이를 아주 길고 상세하게 작성해줘.`;
            ...
            if (result.candidates && result.candidates.length > 0) {
                // gemini-2.5-flash는 일반 텍스트 배열을 반환합니다. 
                const parts = result.candidates[0].content.parts;
                // thought가 아닌 실제 text를 포함하는 part를 찾습니다.
            ...
            setErrorMsg(error.message || "우주의 기운이 혼탁하여 천명을 읽지 못했습니다. 잠시 후 다시 시도해주세요.");
            setView('form');
        }
    };
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center p-4 pt-12 relative z-10">
            <div className="text-center mb-12">
                <h1 className="text-5xl md:text-6xl font-serif text-gold-gradient font-extrabold tracking-widest mb-2 drop-shadow-lg">
                    天命
                </h1>
            ...
            {view === 'form' && <UserInputForm onSubmit={fetchFortuneWithAI} externalError={errorMsg} />}
            {view === 'loading' && <LoadingScreen />}
            {view === 'result' && aiResult && <ResultDashboard onReset={handleReset} userData={userData} aiResult={aiResult} />}
        </div>
    );
}
